...
// TODO: Realizovati metod koji vraca da li postoji i odredjuje put u grafu izmedju zadatog pocetnog i krajnjeg cvora cija je duzina (broj potega na putu) najmanja.
bool GraphAsListsInt::pathWithMinimalNumberOfEdges(int dataSource, int dataDest)
{
	// postavi status na neobradjen i duzinu na beskonacno
	// i nadji pocetni i krajnji cvor
	LinkedNodeInt* pSrc = nullptr;
	LinkedNodeInt* pDest = nullptr;
	LinkedNodeInt* ptr = start;
	while (ptr != nullptr) {
	...
	que.enqueue(ptr);
	ptr->status = 2; // u strukturi

	while (!que.isEmpty()) {
		ptr = que.dequeue();
		ptr->status = 3; // obradjen

		LinkedEdgeInt* pEdge = ptr->adj;		// za svaki u listi potega
		while (pEdge != nullptr) {

			// ako je nadjen put manje duzine update prev i duzinu
			if (ptr->duzina + 1 < pEdge->dest->duzina)
			{
				pEdge->dest->prev = ptr;
				pEdge->dest->duzina = ptr->duzina + 1;
			}

			if (pEdge->dest->status == 1)
			{
				pEdge->dest->status = 2; // u strukturi
	...
	if (pDest->prev != nullptr) {
		cout << "Put duzine " << pDest->duzina << ":\n";
		LinkedNodeInt* ptrTmp = pDest;
