...
// TODO: Realizovati metod koji odredjuje i prikazuje jedan (bilo koji) put u grafu izmedju zadatog pocetnog i krajnjeg cvora koji prolazi kroz treci zadati cvor i ne mora biti prost.
void GraphAsListsInt::pathThroughNode(int dataFirst, int dataLast, int dataThrough)
{
	...
	while (!stek.isEmpty()) {
		...
		while (pEdge != nullptr) {
			...
			if (pEdge->dest->node == dataThrough) {
