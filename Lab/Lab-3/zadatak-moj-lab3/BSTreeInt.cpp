...
//TODO: Realizovati rekurzivni metod koji za svaka dva čvora C i P, za koje važi da nemaju desne potomke
// i da je P direktni levi potomak C, postavlja levog potomka P na mesto čvora C, zadržavajući levo podstablo potomka P i
// postavljajući čvor C na mesto desnog potomka P // (voditi računa o efikasnosti ovog metoda – ne koristiti metod za dodavanje i brisanje elementa iz stabla). 
//TODO: Rekurzivni metod sa potrebnim parametrima je neophodno dodati u klasu binarnog stabla. 
void BSTreeInt::updateAllSpecificPairs()
{
	//TODO: Dodati odgovarajući poziv realizovanog metoda

	// spec slucaj za koren
	if (nemaDesnog(root) && nemaDesnog(root->left)) {
		BSTNodeInt* tmp = root;
		root = root->left;
		tmp->left = nullptr;
		root->right = tmp;
	...
	updateAllSpecificPairs(root->left, root, 0);
	updateAllSpecificPairs(root->right, root, 1);
}

void BSTreeInt::updateAllSpecificPairs(BSTNodeInt* node, BSTNodeInt* root, bool strana)
{
	if (root == nullptr || node == nullptr || node->left == nullptr)
		return;

	if (nemaDesnog(node) && nemaDesnog(node->left))
	{
		BSTNodeInt* P = node->left;
		BSTNodeInt* C = node;
	...
	updateAllSpecificPairs(node->left, node, 0);
	updateAllSpecificPairs(node->right, node, 1);
}

bool BSTreeInt::nemaDesnog(BSTNodeInt* node)
{
	if (node == nullptr)
		return false;
