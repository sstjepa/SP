...
{
	// metoda koja proverava da li je stablo perfektno balansirano
	int tmp = isPerfectHelper(root);
	if (tmp == -1)
		return false;
	return true;
}

int BSTreeInt::isPerfectHelper(BSTNodeInt* node)
{
	// ako je prazno (pod)stablo
	if (node == nullptr)
		return 0;
	...
	int left = isPerfectHelper(node->left);
	int right = isPerfectHelper(node->right);

	// ako je neko od podstabala neperfektno vracaj -1 kroz pozive uvek
	// jer za svaki cvor mora da vazi perfektnost
	if (left == -1 || right == -1)
		return -1;

	// ako je podstablo sa trenutnim cvorem perfektno
	// vrati broj cvorova u tom podstablu + 1 (trenutni cvor)
	// posle se taj broj poredi sa brojem cvorova u drugom podstablu
	// i sve tako na gore kako je rekurzija krenula
	if (abs(left-right) <= 1)
		return 1 + left + right;
	
	// ako je razlika veca od 1 vrati -1 (neperfektno)
	return -1;
}
