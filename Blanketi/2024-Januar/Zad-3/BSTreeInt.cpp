...
int BSTreeInt::countCloseParents(BSTNodeInt* root, int n)
{
	...
	{
		if (abs(root->getKey() - root->left->getKey()) <= n &&
			abs(root->getKey() - root->right->getKey()) <= n )
		{
			return 1 + countCloseParents(root->left, n) + countCloseParents(root->right, n);
		}
	}

	return countCloseParents(root->left, n) + countCloseParents(root->right, n);
}
