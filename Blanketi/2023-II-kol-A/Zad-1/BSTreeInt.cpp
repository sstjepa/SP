...
int BSTreeInt::maxAbs(BSTNodeInt* node, int& maxAbsValue, BSTNodeInt** maxAbsNode)
{
	...
	// rekurzivno pozovi za levo i desno podstablo
	...
	// azuriraj maksimalnu vrednost ako je potrebno
	if (abs(right - left) > maxAbsValue) {
		maxAbsValue = abs(right - left);
		*maxAbsNode = node;
	}

	// vrati broj cvorova levo i desno i 1 za trenutni cvor
	return left + right + 1;
}
